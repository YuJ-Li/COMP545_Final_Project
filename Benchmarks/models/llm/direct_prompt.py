...
class DirectPrompt:
    """Dependency-light forecaster (LLM Direct Prompt)."""
    def __init__(self, model: str, use_context: bool=True, n_retries: int=3,
                 temperature: float=1.0, dry_run: bool=False, constrained_decoding: bool=False):
        self.model = model
    ...
    def __call__(self, task_instance, n_samples: int = 8) -> Tuple[np.ndarray, dict]:
        ...
        if self._pipe is None:
            ...
            out = self._pipe(
                ...
                do_sample=True,
                num_return_sequences=max(n_samples, 1),
        ...
        arr = np.array(samples, dtype=float)[:, :, None]
